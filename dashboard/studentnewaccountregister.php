				...
				<div class="row">
					<div class="col-md-12">
						<div class="header-content">
							<h1 style="color:black;"> New Student Account</h1>
...
<?php
	...
	if (!isset($_POST ['StudentUsername'], $_POST ['StudentPassword'], $_POST ['StudentVolunteerEmail'] ))
	{
		// Could not get the data that should have been sent.
		die ('Please complete the registration form!');
	}
	// Make sure the submitted registration values are not empty.
	if (empty($_POST['StudentUsername']) || empty($_POST['StudentPassword']) || empty($_POST['StudentVolunteerEmail']))
	{
		// One or more values are empty.
		die ('Please complete the registration form');
	}
	if (!filter_var($_POST['StudentVolunteerEmail'], FILTER_VALIDATE_EMAIL))
	{
		die ('Email is not valid!');
	}
	if (preg_match('/[A-Za-z0-9]+/', $_POST['StudentUsername']) == 0)
	{
		die ('StudentUsername is not valid!');
	}
	if (strlen($_POST['StudentPassword']) > 20 || strlen($_POST['StudentPassword']) < 5)
	{
		die ('StudentPassword must be between 5 and 20 characters long!');
	}
	
	
	// We need to check if the account with that StudentUsername exists.
	if ($stmt = $con->prepare('SELECT StudentVolunteerID, StudentPassword FROM studentvolunteerusernames WHERE StudentUsername = ?'))
	{
		// Bind parameters (s = string, i = int, b = blob, etc), hash the StudentPassword using the PHP StudentPassword_hash function.
		$stmt->bind_param('s', $_POST['StudentUsername']);
		$stmt->execute();
		$stmt->store_result();
		...
		if ($stmt->num_rows > 0)
		{
			...
			echo 'StudentUsername exists, please choose another!';
		}
		else
		{
			// StudentUsername doesnt exists, insert new account
			if ($stmt = $con->prepare('INSERT INTO studentvolunteerusernames (StudentUsername, StudentPassword, StudentVolunteerEmail) VALUES (?, ?, ?)'))
			{
				// We do not want to expose passwords in our database, so hash the password and use password_verify when a user logs in.
				$StudentPassword = password_hash($_POST['StudentPassword'], PASSWORD_DEFAULT);
				$stmt->bind_param('sss', $_POST['StudentUsername'], $StudentPassword, $_POST['StudentVolunteerEmail']);
				$stmt->execute();
